...
def sk_LDA(corpus, n_topic):
    en_stops = list(
        set(stopwords.words("english")).union(
            set(["jpg", "src", "png", "mp4", "mp3", "ref", "url", "pdf"])
        )
    )
    ...
    lda_model = LatentDirichletAllocation(
        n_components=n_topic,  # Number of topics
...
def compute_coherence_values(corpus, k):
    ...
    def compute_coherence(lda_model, vectors, vocab):
        return tmtoolkit.topicmod.evaluate.metric_coherence_gensim(
            measure="u_mass",
            top_n=25,
    ...
    coherence_model_lda = compute_coherence(
        lda_model, vectors, count_vector.vocabulary_
    )
...
def get_corpus(channel):
    ...
    messages = [
        ...
        for key in messages
        if len(messages[key]["message"]) > 25
    ]
...
def perform_preprocessing(portion_size=1000, n_pool=2):
    ...
    for i, portion in tqdm.tqdm(enumerate(portions), total=len(portions)):
        ...
        with Pool(n_pool) as pool:
            for single_corpus, _id, s_discarded_messages, ok_messages in pool.map(
                get_corpus, portion
            ):
        ...
        save_as_pickle(
            id_list, f"ids_list_topic_modeling/n_gram_ids_list_topic_modeling_{i}"
        )
        save_as_pickle(
            discarded_messages,
            f"discarded_messages_topic_modeling/n_gram_discarded_messages_topic_modeling_{i}",
        )
        ...
        docs = []
        with Pool(n_pool) as pool:
            for channel_tokens in tqdm.tqdm(
                pool.imap(preprocess, all_messages), total=len(all_messages)
            ):
...
def perform_LDA(n_portions=20, min_topics=10, max_topics=31, step_size=1):
    ...
    for idx in tqdm.tqdm(range(n_portions)):
        ...
        new_id_list = open_pickle(
            f"ids_list_topic_modeling/n_gram_ids_list_topic_modeling_{idx}"
        )
